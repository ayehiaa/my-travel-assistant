...
describe('parseReceiptResponse', () => {
  it('valid full JSON → all five fields populated correctly', () => {
  ...
  it('literal string "null" → all-null record', () => {
    ...
    expect(result).toEqual({ title: null, amount: null, currency: null, date: null, category: null })
  })
  ...
  it('markdown-fenced JSON → fences stripped, fields parsed correctly', () => {
    const input = '```json\n{"title":"Nero","amount":3.50,"currency":"GBP","date":"2026-03-15","category":"Meals"}\n```'
    const result = parseReceiptResponse(input)
    expect(result).toEqual({
      title: 'Nero',
      amount: 3.5,
  ...
  it('category string → trimmed and returned', () => {
    const input = JSON.stringify({ title: 'Premier Inn', amount: 120, currency: 'GBP', date: '2026-05-01', category: '  Accommodation  ' })
    const result = parseReceiptResponse(input)
    expect(result.category).toBe('Accommodation')
  })

  it('category > 100 chars → null', () => {
    const input = JSON.stringify({ title: 'Shop', amount: 10, currency: 'GBP', date: '2026-01-01', category: 'A'.repeat(101) })
    const result = parseReceiptResponse(input)
    expect(result.category).toBeNull()
  })

  it('category absent from response → null', () => {
    const input = JSON.stringify({ title: 'Shop', amount: 10, currency: 'GBP', date: '2026-01-01' })
    const result = parseReceiptResponse(input)
    expect(result.category).toBeNull()
  })
})
